...
class VAE_Model(nn.Module):
    def __init__(self, args):
        ...
        self.scheduler  = optim.lr_scheduler.MultiStepLR(self.optim, milestones=[80], gamma=0.1)
    ...
    def training_stage(self):
        ...
        for i in range(self.args.num_epoch):
            ...
            for (img, label) in (pbar := tqdm(train_loader, ncols=120)):
                ...
                MSELoss, KLLoss = self.training_one_step(img, label, adapt_TeacherForcing)
                TotalMSELoss += MSELoss
                TotalKLLoss += KLLoss
            ...
            writer.add_scalar('Train/Teacher_forcing_ratio',self.tfr, self.current_epoch)
            writer.add_scalar('Train/ReConstructLoss',MSELoss, self.current_epoch)
            writer.add_scalar('Train/ReparameterLoss',KLLoss, self.current_epoch)
            writer.add_scalar('Train/TotalLoss',loss, self.current_epoch)
            writer.add_scalar('Train/KL_Annealing_ratio',beta, self.current_epoch)


            loss, PSNRList = self.eval()
            writer.add_scalar('Valid/ValidationLoss',loss, self.current_epoch)
            writer.add_scalar('Valid/ValidationAvgPSNR',sum(PSNRList)/len(PSNRList), self.current_epoch)
            self.current_epoch += 1
            self.scheduler.step()
            self.teacher_forcing_ratio_update()
            self.kl_annealing.update()
        
        np.save('Cyclic_valid_PSNR.npy',np.array(PSNRList))
    ...
    def eval(self):
        ...
        for (img, label) in (pbar := tqdm(val_loader, ncols=120)):
            img = img.to(self.args.device)
            label = label.to(self.args.device)
            loss, PSNRList= self.val_one_step(img, label)
            self.tqdm_bar('val', pbar, loss.detach().cpu(), lr=self.scheduler.get_last_lr()[0])

            print("PSNR Avg results: ",sum(PSNRList)/len(PSNRList))

        return loss, PSNRList
    ...
    def val_one_step(self, img, label):
        ...
        frameList = []
        PSNRList = []
        for frame in range(1, self.args.val_vi_len):
            z = torch.cuda.FloatTensor(1, self.args.N_dim, self.args.frame_H, self.args.frame_W).normal_()
            img_encode = self.frame_transformation(out)
            label_encode = self.label_transformation(label[frame])
            out = self.Generator(self.Decoder_Fusion(img_encode,label_encode,z))
            Loss += self.mse_criterion(out,img[frame])
            frameList.append(out.squeeze())
            PSNRList.append(Generate_PSNR(out, img[frame]).item())
    ...
    def teacher_forcing_ratio_update(self):
        ...
        if self.current_epoch >= self.tfr_sde:
            self.tfr = self.tfr_d_step * self.tfr
    ...
    def load_checkpoint(self):
        if self.args.ckpt_path != None:
            ...
            self.optim      = optim.ASGD(self.parameters(), lr=1e-4)
            self.scheduler  = optim.lr_scheduler.MultiStepLR(self.optim, milestones=[60], gamma=0.1)
